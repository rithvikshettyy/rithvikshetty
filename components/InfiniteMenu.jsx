...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    ...
    // Face the first item (instance 0 → items[0]) toward the viewer on load,
    // so the sphere always opens centered on project 1 instead of a random one.
    if (this.instancePositions.length) {
      const firstDir = vec3.normalize(vec3.create(), this.instancePositions[0]);
      quat.rotationTo(this.control.orientation, firstDir, this.control.snapDirection);
    }
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div className="infinite-menu-wrap">
      ...
      {/* Edge scrims — keep the side captions legible and separated from any
          orbiting discs at the sphere's left/right edges */}
      <div className="im-edge-scrim" aria-hidden="true" />

      {activeItem && (
        <>
          <div className={`face-caption ${isMoving ? 'inactive' : 'active'}`}>
            <span className="face-index">Project / {String(activeIndex + 1).padStart(2, '0')}</span>
