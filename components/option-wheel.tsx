...
// OptionWheel (React Bits) — a curved, scrollable option picker. Options sit
// on a circle; wheel/drag/arrow keys ease the wheel toward a target index.
...
type OptionWheelProps = {
  ...
  soundVolume?: number
  className?: string
  /** Externally controlled index — the wheel eases toward it when it changes. */
  selected?: number | null
  /** false = display-only: no wheel hijack, drag, keys, or item clicks. */
  interactive?: boolean
  /** Continuous external drive (duck-typed framer MotionValue): fractional
      positions stream in without re-rendering React — silky scroll-scrub. */
  progressValue?: { on(event: 'change', cb: (v: number) => void): () => void } | null
}
...
const OptionWheel = ({
  ...
  // External control — ease toward the given index whenever it changes.
  useEffect(() => {
    if (selected == null) return
    applyTarget(selected, true)
  }, [selected, applyTarget])

  // Continuous external drive — fractional targets stream straight into the
  // easing loop, bypassing React state entirely.
  useEffect(() => {
    if (!progressValue) return
    return progressValue.on('change', (v: number) => applyTarget(v, false))
  }, [progressValue, applyTarget])
  ...
  }, [applyTarget, interactive])

  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    if (!cfgRef.current.interactive || !cfgRef.current.draggable) return
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      if (!cfgRef.current.interactive || dragMovedRef.current) return
      const cfg = cfgRef.current
